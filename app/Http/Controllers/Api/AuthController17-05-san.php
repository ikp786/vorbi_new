...
// 				//city  wise user
// 				$matchedUsers = User::where("city", auth()->user()->id)->where('id', "!=", $primaryId)->whereNotIn("id",$historyCall)->inRandomOrder()->first();
// 				if (empty($matchedUsers)) {
// 					$matchedUsers = User::where("language_id", auth()->user()->language_id)->where('id', "!=", $primaryId)->whereNotIn("id",$historyCall)->inRandomOrder()->first();
// 					if (empty($matchedUsers)) {
// 						return $this->sendFailed('Sorry!! Currently no users are available, will send a notification the moment we have some available', 201);
// 					}
// 				}
// 			}
		...
		$response2 = Http::get('https://aksasoftware.com/vorbi/public/zeocloud/test/test.php?user_id=' . $unique_id . '&streaming_id=' . $streaming_id . '&room_id=' . $roomId);
		...
		return $this->sendSuccess('TOKEN GENERATE SUCCESSFULLY!', [
			'room_id' => $roomId, 'unique_id' => $matchedUsers->unique_id,
	...
	function saveRatting(Request $request)
	{
		...
		$userfirst->zeo_token=null;
		$userfirst->save();

		$usersec=User::find( $request->to_user_id);
		$usersec->zeo_token=null;
		$usersec->save();
	...
	public function sendVoip(Request $request)
	{
		...
		$payload = [
			...
			'user_id'	 	 => $loginUserId,
			'token'			 =>  $LoginUserDetail->zeo_token,
...
// 		$url = 'https://api.development.push.apple.com/3/device/' . $userDetail->device_token;
		...
		return $this->sendSuccess('TOKEN GENERATE SUCCESSFULLY!', [
			'id' => $userDetail->id,
			'unique_id' => $userDetail->unique_id, 'token' => $userDetail->zeo_token
		]);
	}
