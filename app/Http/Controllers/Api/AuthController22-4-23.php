...
class AuthController extends Controller
{
 ...
 public function __construct()
	...
	function sentVoipToken(Request $request){
		...
		return $this->sendSuccess('TOKEN GENERATE SUCCESSFULLY!', ['room_id' => $roomId,
		'id' => $matchedUsers->$id,
		'unique_id' => $matchedUsers->unique_id, 'token' => $myData->zeo_token,'device_token' => $device_token]);
	}

	public function testnotification($roomId,$deviceToken,$userId,$token,$device_token,$type)
	{
...
return $this->sendSuccess('HISTORY GET SUCCESSFULLY',['data' =>UserProfileCollection::collection($users)]);
...
// 		$allCalls = auth()->user()->allCallsId($today)		
// 		->pluck('callee_id');		
...
$matchedUsers = auth()->user()->whereHas('topics', function($query) use ($authUserTopicIds, $allCalls) {
	$query->whereIn('topic_id', $authUserTopicIds);
// 	->whereNotIn('user_id',$allCalls);
 })
 ->first();
		if(!isset($matchedUsers->id)){
			return $this->sendFailed('NOT USER FOUND',201);
		}
		...
		$response = Http::get('https://aksasoftware.com/vorbi/public/zeocloud/test/test.php?user_id='.$unique_id.'&streaming_id='.$streaming_id.'&room_id='.$roomId);

		$token = json_encode($response->json()['token']);
		// dd($token,$unique_id);
		
		$token = str_replace('"', '', $token);
		$token = str_replace('"', '', $token);
		// $unique_id = auth()->user()->unique_id;
		$response2 = Http::get('https://aksasoftware.com/vorbi/public/zeocloud/test/test.php?
		user_id='.$matchedUsers->unique_id.'
		
		&streaming_id='.$streaming_id.'&room_id='.$roomId);
		$token2 = json_encode($response2->json()['token']);
		...
		$streaming_id 	= $kitToken['stream_id_list'];
		$matchedUsers2 = User::find($matchedUsers->id);
		$matchToken = $matchedUsers2->zeo_token;
		$device_token = $matchedUsers2->device_token;
        self::testnotification($roomId,$deviceToken,$user_id,$matchToken,auth()->user()->device_token,'');
        $myData = User::find(auth()->user()->id);

		

return $this->sendSuccess('TOKEN GENERATE SUCCESSFULLY!', ['room_id' => $roomId, 'unique_id' => $matchedUsers->unique_id,  
'user_id' => $matchedUsers->id,
'token' => $myData->zeo_token,'device_token' => $device_token]);
		//END
